...
namespace bumperbot_mapping
{
    ...
    MappingWithKnownPoses::MappingWithKnownPoses(const std::string & name) : rclcpp::Node(name)
    {
        ...
        // Identity orientation for the map frame (no rotation w.r.t. odom).
        map_.info.origin.orientation.w = 1.0;
        map_.header.frame_id = "odom";
        map_.data = std::vector<int8_t>(map_.info.width * map_.info.height, -1);

        // Publish the OccupancyGrid on /map with a QoS compatible with RViz and nav2_map_server:
        // - Reliable
        // - KeepLast(1)
        // - Transient Local (latched map)
